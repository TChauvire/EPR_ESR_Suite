...
folder = 'D:\\Documents\\Recherches\\Postdoc\\Data\\CraneLab_Flavoprotein\\'\
    'Sidddarth\\2019\\Apr23_iLAW_iLov_EcA_EcW_BL21\\T1\\'
...
for i in range(len(ListOfFiles)):
    ...
    line2, = plt.plot(FitValue[:, 5*i], FitValue[:, 5*i+1], 'r',
                      label='monoexponential fit')
    line3, = plt.plot(FitValue[:, 5*i], FitValue[:, 5*i+3], 'b',
                      label='biexponential fit')
    line4, = plt.plot(Y[0:npts, 4*i], Y[0:npts, 4*i+2], 'k--', linewidth=2,
                      label='imaginary part')
    plt.xlabel('Time [us]', fontsize=16, fontweight='bold')
