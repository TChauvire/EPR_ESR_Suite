...
for j in range(len(fullname)):
    ...
    axes[0].plot(t, newy, 'k', t, np.dot(K, Pr.T[:, i]), 'r')
    axes[1].plot(RMSD[:, j])
    axes[2].plot(diff[:, j])
    ...
    axes[3].plot(r, Pr.T[:, idx[j]+1], 'C1',
                 label='SVDCutoff={0}'.format(idx[j]+1))
    axes[3].plot(r[maxiP1[j][0],], maxiP1[j][1], '+', label=str())
    axes[3].plot(r, Pr.T[:, idx[j]+2], 'C2',
                 label='SVDCutoff={0}'.format(idx[j]+2))
    axes[3].plot(r[maxiP2[j][0],], maxiP2[j][1], '+', label=str())
    axes[3].plot(r, Pr.T[:, idx[j]+3], 'C3',
                 label='SVDCutoff={0}'.format(idx[j]+3))
    axes[3].plot(r[maxiP3[j][0],], maxiP3[j][1], '+', label=str())
    axes[3].plot(r, Pr.T[:, idx[j]+4], 'C4',
                 label='SVDCutoff={0}'.format(idx[j]+4))
    axes[3].plot(r[maxiP4[j][0],], maxiP4[j][1], '+', label=str())
    axes[3].plot(r, Pr.T[:, idx[j]+5], 'C5',
                 label='SVDCutoff={0}'.format(idx[j]+5))
    axes[3].plot(r[maxiP5[j][0],], maxiP5[j][1], '+', label=str())
